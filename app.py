...
import os, random, colorsys, re, time
import psycopg2
from psycopg2.extras import RealDictCursor
from collections import defaultdict

app = Flask(__name__)
app.secret_key = 'super-secret-key'  # Prod'da ortam değişkenine taşımanı öneririm
...
# Bakım Modu Ayarları
# ----------------------------
# MAINTENANCE_MODE=1 -> bakım açık (503 + maintenance.html)
# MAINTENANCE_ALLOW_IPS="1.2.3.4,5.6.7.8" -> bu IP'ler bakım modunu baypas eder
# MAINTENANCE_BYPASS_TOKEN="uzun-bir-gizli-token" -> /__maintenance-bypass?token=... ile cookie baypas
...
def maintenance_bypass_ok():
    ...
    return bool(token) and request.cookies.get(MAINTENANCE_COOKIE) == token
...
def get_db_connection():
    return psycopg2.connect(
        dbname="railway",
        user="postgres",
        password="BuYuHoBHNkQNGxbQHDGNCVrYtnLWhIvo",  # Prod'da ENV'e taşı!
...
def _maintenance_guard():
    # Açık kalan yollar: health, bypass, maintenance sayfası, favicon/robots, statikler
    open_paths = (
        "/healthz",
    ...
    if maintenance_active():
        ip = client_ip()
        if maintenance_allowed_ip(ip) or maintenance_bypass_ok():
            return  # izinli -> devam
        # değilse: 503 + maintenance.html
...
def maintenance_bypass():
    """ Gizli token doğrulanırsa bypass cookie'si set edilir.
    ...
    # secure=True'ı prod'da (HTTPS) aktifleştir
    resp.set_cookie(MAINTENANCE_COOKIE, token, httponly=True, samesite="Lax", secure=False, max_age=60*60*3)
    return resp
